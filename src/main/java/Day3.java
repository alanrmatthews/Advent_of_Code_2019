...
public class Day3 {
    ...
    private static HashSet<PointDist> parsePath(String[] path) {
        HashSet<PointDist> output = new HashSet<>();
            ...
            switch (direction) {
                case 'U':
                    for (int x = row + 1; x <= row + length; x++)
                        output.add(new PointDist(x, col, ++distance));
                    row += length;
                    break;
                case 'D':
                    for (int x = row - 1; x >= row - length; x--)
                    ...
                    break;
                case 'L':
                    for (int x = col - 1; x >= col - length; x--)
                        output.add(new PointDist(row,x, ++distance));
                    col -= length;
                    break;
                case 'R':
                    for (int x = col + 1; x <= col + length; x++)
    ...
    private static class PointDist extends IntPair {
        private final int Distance;

        public PointDist(int L, int R, int Dist) {
            super(L, R);
            Distance = Dist;
        }
